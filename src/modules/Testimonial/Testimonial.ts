...
const createTestimonail = async (
  ...
  try {
    ...
    if (image && image?.buffer !== "" && Object.entries(image || {}).length) {
      const uploaded = await uploadFile(req.body.image);

      createdTestimonial.image = {
        name: image?.filename,
        url: uploaded.url,          // ✅ STRING
        public_id: uploaded.public_id, // ✅ STORE THIS
        type: image?.type,
      };
...
export const updateTestimonial = async (req: any, res: any): Promise<any> => {
  try {
    const data = req.body
    ...
    if (testimonial) {
      ...
      if (
        ...
        await deleteFile(updatedData.image.public_id);
      ...
      if (
        ...
        const uploaded = await uploadFile(data.image);
...
export const deleteTestimonial = async (req: any, res: Response) => {
  try {
    ...
    if (test) {
      if (test?.image?.name) {
        await deleteFile(test.image.public_id);
      }
      return res.status(statusCode.success).send({
        status: "success",
        data: test,
